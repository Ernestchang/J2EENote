...
<body>
	...
	<div class="container">
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h3 class="panel-title text-center">未审核的算法列表</h3>
			</div>
			<table class="table table-hover table-bordered table-condensed">
				<tr><th>算法名</th><th colspan="2" class="text-center">操作</th></tr>
				<c:forEach items="${list}" var="entry">
					<tr>
						<td><a target="_blank" href="${pageContext.request.contextPath }/algorithm/detail/${entry.id}">${entry.name }</a></td><td><button type="button" onclick="javascript:changeStatus('${entry.id}',3)" class="btn btn-success btn-sm">标记为通过</button></td><td><button type="button" onclick="javascript:changeStatus('${entry.id}',2)" class="btn btn-primary btn-sm">标记为未通过</button></td>
					</tr>
				</c:forEach>
			</table>
		</div>
	...
	<script type="text/javascript" src="${pageContext.request.contextPath }/resources/moderator/js/algorithm/notvertifylist.js"></script>
</body>
</html>