...
public class UserController {
	...
	@RequestMapping(value = "/auth/apply", method = RequestMethod.POST, produces = "text/plain;charset=UTF-8")
	public @ResponseBody
	String apply(@Valid ApplyDto applyDto, BindingResult bindingResult, HttpSession session) {
		AjaxObj ajaxObj = new AjaxObj();
		if (bindingResult.hasErrors()) {
		...
		if (!applyDto.getVcode().equalsIgnoreCase((String) session.getAttribute("vcode"))) {
			ajaxObj.setSuccess(false);
			ajaxObj.setMsg("验证码输入错误");
		} else {
			@SuppressWarnings("unchecked")
			Map<String,Object> loginUser = (Map<String, Object>) session.getAttribute("loginUser");
			applyDto.setUid((String) loginUser.get("id"));
			if (userService.apply(applyDto)) {
				ajaxObj.setSuccess(true);
			} else {
				ajaxObj.setSuccess(false);
